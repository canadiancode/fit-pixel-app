...
import { getEarliestSleepDayKey, getSleepHistorySeries } from "@/lib/db";

type SleepHistoryChartState = {
...
export function useSleepHistoryChart(
  lookback: HistoryLookbackId,
): SleepHistoryChartState {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const earliestSleepDayKey =
          lookback === "allTime" ? await getEarliestSleepDayKey(db) : null;
        if (cancelled) return;

        const spec = historySpecForLookback(lookback, earliestSleepDayKey);
        if (spec === "empty") {
          setUserData(EMPTY_WEEK);
          return;
        }

        const series = await getSleepHistorySeries(db, spec);
        if (cancelled) return;
