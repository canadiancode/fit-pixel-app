...
type CaloriesHistoryChartState = {
...
export function useCaloriesHistoryChart(
  lookback: HistoryLookbackId,
): CaloriesHistoryChartState {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const earliestActiveKcalDayKey =
          lookback === "allTime" ? await getEarliestActiveKcalDayKey(db) : null;
        if (cancelled) return;

        const spec = historySpecForLookback(lookback, earliestActiveKcalDayKey);
        if (spec === "empty") {
          setUserData(EMPTY_WEEK);
          return;
        }

        const series = await getActiveKcalHistorySeries(db, spec);
        if (cancelled) return;
